...
from src.models.schemas.product import ProductUpdate, ProductBase
...
class ProductService(BaseService[Product]):
    async def create(self, organization_id: str, data: List[ProductBase]) -> List[Product]:
        results = []
    ...
    async def update(self, organization_id: str, product_id: str, data: ProductUpdate) -> Product:
    ...
    async def get_by_id(self, product_id: str, organization_id: str) -> Optional[Product]:
    ...
    async def delete(self, organization_id: str, product_id: str) -> bool:
        product = await self.get_by_id(product_id, organization_id)
        if not product:
            raise HTTPException(status_code=404, detail="Product not found")
