...
class PayOrderService(BaseService[PayOrder]):
    ...
    async def payment_details(self, payorder_id: str, req: PaymentDetailsRequest) -> PaymentDetailsResponse:
        ...
        async with ChangeNowService() as cn:
            print(f"~~~ Exchange ~~~\n")
            print(f"\taddress= {pay_order.destination_receiving_address}")
            print(f"\trefund_address= {req.refund_address}")
            print(f"\tamount= {quote.in_currency.ui_amount}")
            print(f"\tcurrency_in= {quote.in_currency.name}")
            print(f"\tcurrency_out= {quote.out_currency.name}")
            print(f"~~~~~~~~~~~~~~~~~\n\n")
