...
if __name__ == "__main__":
    logger.info("Starting ChangeNow API test")
    
    try:
        client = ChangeNowClient()
        
        # Test exchange estimation
        ...
        estimate = client.get_estimated_exchange_amount(estimate_request)
        logger.success(f"Received estimate: {estimate.model_dump_json(indent=2)}")
        ...
        #    refundAddress="BNd67okqyiHWELazru4XdkAZb7ENnursaqNAtWYBMUEo",
        #    toAmount="0.1"
        #)
        #
        #exchange = client.create_exchange_transaction(exchange_request)
        #logger.success(f"Exchange created: {exchange.model_dump_json(indent=2)}")

        ## Test exchange status
        #status = client.get_exchange_status(exchange.id)
        #logger.success(f"Exchange status: {status.model_dump_json(indent=2)}")
