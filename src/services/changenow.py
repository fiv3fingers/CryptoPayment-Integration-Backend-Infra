...
class ChangeNowService:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise ValueError(
                "API key must be provided or set in CHANGENOW_API_KEY environment variable"
            )
    ...
    @cached(
        ttl=900, cache=Cache.MEMORY, key_builder=get_currencies_cache_key  # 15 minutes
    )
    async def get_available_currencies(
        ...
        try:
            ...
            async with self.session.get(
                f"{self.base_url}/exchange/currencies", params=params
            ) as response:
    ...
    @cached(ttl=60, cache=Cache.MEMORY, key_builder=get_estimate_cache_key)  # 1 minute
    async def _create_estimate(self, request: EstimateRequest) -> Estimate:
        ...
        try:
            ...
            async with self.session.get(
                f"{self.base_url}/exchange/estimated-amount", params=params
            ) as response:
                response.raise_for_status()
                data = await response.json()
    ...
    async def _create_exchange(self, request: ExchangeRequest) -> Exchange:
        ...
        try:
            ...
            async with self.session.post(
                f"{self.base_url}/exchange", json=data
            ) as response:
                response.raise_for_status()
                data = await response.json()
    ...
    async def get_exchange_status(self, exchange_id: str) -> ExchangeStatus:
        ...
        try:
            async with self.session.get(
                f"{self.base_url}/exchange/by-id", params={"id": exchange_id}
            ) as response:
                response.raise_for_status()
                data = await response.json()
    ...
    @cached(
        ttl=900, cache=Cache.MEMORY, key_builder=get_currency_cache_key  # 15 minutes
    )
    async def _get_changenow_currency(
        ...
        try:
            ...
            relevant_currencies = [
                c for c in cn_currencies if c.network == network_name
            ]
            ...
            if currency.is_native:
                result = next(
                    (c for c in relevant_currencies if c.token_contract is None), None
                )
            else:
                result = next(
                    (
                        c
                        for c in relevant_currencies
                        if c.token_contract
                        and c.token_contract.lower() == currency.address.lower()
                    ),
                    None,
                )
