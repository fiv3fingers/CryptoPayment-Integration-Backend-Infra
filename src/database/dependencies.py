...
def get_current_organization(
    ...
    organization = (
        db.query(Organization).filter(Organization.api_key == api_key).first()
    )
...
def validate_authorization_header(
    auth_header: str = Depends(authorization_header), db: Session = Depends(get_db)
) -> Organization:
    header_parts = parse_header(auth_header)
    organization = (
        db.query(Organization)
        .filter(Organization.api_key == header_parts.get("APIKey"))
        .first()
    )
