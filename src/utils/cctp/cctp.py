...
class CCTPBridge:
        ...
        for chain in Chain:
            try:
                ...
                self.contracts[chain] = {
                    "token_messenger": self._load_contract(
                        w3, config.token_messenger_address, TOKEN_MESSENGER_ABI
                    ),
                    "usdc": self._load_contract(w3, config.usdc_address, USDC_ABI),
                    "message_transmitter": self._load_contract(
                        w3, config.message_transmitter_address, MESSAGE_TRANSMITTER_ABI
                    ),
                }
    ...
    def _load_contract(w3: Web3, address: str, abi: str) -> Contract:
        return w3.eth.contract(
            address=to_checksum_address(address), abi=json.loads(abi)
        )
    ...
    def get_usdc_allowence(self, chain: Chain, account: LocalAccount) -> int:
        ...
        try:
            return usdc_contract.functions.allowance(
                account.address, config.token_messenger_address
            ).call()
        except Exception as e:
    ...
    def approve_usdc(self, chain: Chain, account: LocalAccount, amount: int) -> str:
        ...
        try:
            ...
            approve_tx = usdc_contract.functions.approve(
                config.token_messenger_address, amount
            ).build_transaction(
                {
    ...
    def get_attestation(
        self, message_hash: bytes, max_attempts: int = 60
    ) -> Optional[str]:
        ...
        while attempts < max_attempts:
            try:
                response = requests.get(
                    f"{ATTESTATION_BASE_URL}0x{message_hash.hex()}", timeout=10
                )
    ...
    def complete_transfer(
        ...
        try:
            ...
            signed_tx = w3.eth.account.sign_transaction(
                receive_tx, destination_account.key
            )
            tx_hash = w3.eth.send_raw_transaction(signed_tx.raw_transaction)
            receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
...
def verify_attestation(message_hash: str) -> Dict[str, any]:
    ...
    """
    try:
        response = requests.get(f"{ATTESTATION_BASE_URL}{message_hash}", timeout=10)
        response.raise_for_status()
